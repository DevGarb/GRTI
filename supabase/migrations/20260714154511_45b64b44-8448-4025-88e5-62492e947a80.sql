-- TMA: remover fallback para closed_at (só conta tickets que passaram por "Aguardando Aprovação")
...
BEGIN
  ...
  -- TMA: 1ª "Aguardando Aprovação" - started_at. Sem fallback para closed_at.
  finished AS (
  ...
  handle AS (
    SELECT c.assigned_to,
      AVG(EXTRACT(EPOCH FROM (f.at - c.started_at)) / 60.0)::numeric AS mins
    FROM closed c
...
BEGIN
  ...
  -- TMA sem fallback: só entra quem passou por "Aguardando Aprovação"
  finished AS (
    SELECT h.ticket_id, MIN(h.created_at) AS at
    FROM public.ticket_history h
    WHERE h.action = 'status_change'
  ...
  work_min AS (
    SELECT c.id AS ticket_id,
      (EXTRACT(EPOCH FROM (f.at - c.started_at)) / 60.0)::numeric AS mins
    FROM closed c
    JOIN finished f ON f.ticket_id = c.id
    WHERE c.started_at IS NOT NULL AND f.at > c.started_at
  ),
