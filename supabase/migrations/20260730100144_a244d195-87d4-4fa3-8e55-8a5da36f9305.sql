...
BEGIN
  ...
  LOOP
    ...
    SELECT _tpl, _organization_id,
           ...
           GREATEST(1, LEAST(3, round(COALESCE(it.weight, 1))))::smallint,
           false,
...
DECLARE
  ...
  _tpl uuid;
BEGIN
  SELECT id INTO _org FROM public.organizations WHERE slug = 'grcheck';
  IF _org IS NULL THEN RETURN; END IF;

  FOR _c IN SELECT id, name, description, active FROM public.chk_imp_checklists WHERE organization_id = _org
  LOOP
    SELECT t.id INTO _tpl FROM public.chk_templates t
      WHERE t.organization_id = _org AND t.import_checklist_id = _c.id;
    IF _tpl IS NULL THEN
      INSERT INTO public.chk_templates (organization_id, title, description, frequency, is_active, import_checklist_id)
      VALUES (_org, _c.name, _c.description, 'unica', _c.active, _c.id)
      RETURNING id INTO _tpl;
    ELSE
      UPDATE public.chk_templates SET title = _c.name, description = _c.description, is_active = _c.active, updated_at = now() WHERE id = _tpl;
      DELETE FROM public.chk_template_items WHERE template_id = _tpl;
    END IF;
