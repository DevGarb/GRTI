...
DROP FUNCTION IF EXISTS public.get_metas_tecnicos(integer, integer);
...
AS $function$
DECLARE
  start_date timestamptz; end_date timestamptz; org_id uuid;
BEGIN
  ...
  RETURN QUERY
  WITH techs AS (
    SELECT DISTINCT p.user_id, p.full_name FROM public.profiles p
  ...
  closed AS (
    ...
    WHERE t.status = 'Fechado' AND t.closed_at >= start_date AND t.closed_at < end_date
      AND t.assigned_to IS NOT NULL AND t.organization_id = org_id
  ),
  sat AS (
    SELECT c.id AS ticket_id, e.score::numeric AS score
    FROM closed c LEFT JOIN public.evaluations e ON e.ticket_id = c.id AND e.type = 'satisfaction'
  ),
  reworks AS (
    SELECT h.ticket_id, COUNT(*)::int AS rwc FROM public.ticket_history h
    WHERE h.action = 'rework' AND h.ticket_id IN (SELECT id FROM closed) GROUP BY h.ticket_id
  ),
  preventivas AS (
    SELECT pm.created_by AS user_id, COUNT(*)::int AS cnt FROM public.preventive_maintenance pm
    WHERE pm.created_at >= start_date AND pm.created_at < end_date GROUP BY pm.created_by
  ),
  events AS (
  ...
  effective_start AS (
    SELECT c.id AS ticket_id,
      COALESCE(c.started_at,
        (SELECT MIN(e.created_at) FROM events e WHERE e.ticket_id = c.id AND e.new_value = 'Em Andamento'),
        c.created_at) AS at
    FROM closed c
  ),
  rework_min AS (
    ...
    FROM (
      ...
      FROM events
    ) x WHERE open_at IS NOT NULL AND close_at IS NOT NULL GROUP BY ticket_id
  ),
  first_exit AS (
  ...
  initial_min AS (
    SELECT c.id AS ticket_id,
      public.business_minutes_between(es.at, COALESCE(fe.at, c.closed_at)) AS mins
    FROM closed c
    JOIN effective_start es ON es.ticket_id = c.id
    LEFT JOIN first_exit fe ON fe.ticket_id = c.id
  ),
  work_min AS (
  ...
  per_ticket AS (
    SELECT c.assigned_to, c.id, c.title, c.closed_at,
      s.score::numeric AS score, cat.name AS category_name,
  ...
  SELECT
    techs.user_id, COALESCE(techs.full_name, 'Sem nome'),
    COUNT(pt.id)::int,
    ...
    COALESCE(jsonb_agg(jsonb_build_object(
      'title', pt.title, 'score', pt.score,
      'resolution_hours', ROUND((pt.work_minutes / 60.0)::numeric, 2),
      'closed_at', pt.closed_at, 'category_name', pt.category_name, 'points', pt.points
