...
BEGIN
  ...
  SELECT COALESCE(AVG(target_value),0) INTO tma_target_avg_hours
    FROM public.performance_goals
   WHERE metric='tma_hours' AND period='monthly'
     AND reference_month=_month AND reference_year=_year
     AND (organization_id = _organization_id OR organization_id IS NULL);

  SELECT COALESCE(SUM(target_value),0) INTO projects_target_total
    FROM public.performance_goals
   WHERE metric='projects_delivered' AND period='monthly'
     AND reference_month=_month AND reference_year=_year
     AND (organization_id = _organization_id OR organization_id IS NULL);
  ...
  SELECT COALESCE(AVG(score),0), COUNT(*)
   ...
   WHERE t.organization_id = _organization_id
     AND e.type = 'satisfaction'
     AND e.created_at >= _from AND e.created_at < _to;

  SELECT COUNT(*) INTO rework_month
     ...
     AND EXISTS (
       SELECT 1 FROM public.ticket_history h
        WHERE h.ticket_id = t.id
          ...
          AND h.new_value = 'Retrabalho'
     );

  IF closed_month > 0 THEN
    rework_actual_percent := ROUND((rework_month::numeric / closed_month::numeric) * 100, 2);
  END IF;
  ...
  -- TMA real do mês (horas úteis, started_at → 1ª transição para "Aguardando Aprovação").
  -- Sem fallback pra closed_at — tickets sem essa transição não entram na média.
  WITH finished AS (
  ...
  SELECT COUNT(*) INTO projects_actual_total
   ...
   WHERE pt.organization_id = _organization_id
     AND pt.status = 'concluida'
     AND pt.updated_at >= _from AND pt.updated_at < _to;

  SELECT COUNT(*) INTO active_sprints_backlog
    FROM public.sprints s
   WHERE s.organization_id = _organization_id
     AND s.status IN ('planejada','em_andamento');
  ...
  RETURN jsonb_build_object(
    ...
    'csat_actual_avg', ROUND(csat_actual_avg::numeric, 2),
